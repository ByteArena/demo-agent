...
module.exports = function computeAvoidanceForce(perception, specs) {
    ...
    if(perception.vision) {
        ...
        const topleft = bottomleft.clone().rotate(-Math.PI/2).mag(30).add(bottomleft);         // 15 u en avant
        const topright = bottomright.clone().rotate(Math.PI/2).mag(30).add(bottomright);       // 15 u en avant
